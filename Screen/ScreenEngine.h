...
struct ScreenConstructionParameters
{
    ...
    const sf::Texture & menuBackgroundTexture;

    const sf::Texture & activeButtonTexture;

    const sf::Texture & inactiveButtonTexture;

    const sf::Texture & sliderBarTexture;
    
    const sf::Texture & sliderSliderTexture;

    const sf::Texture & sliderActiveSliderTexture;

    const sf::Texture & checkboxBackgroundTexture;

    const sf::Texture & checkboxActiveBackgroundTexture;

    const sf::Texture & checkboxCheckTexture;

    const sf::Texture & sfmlLogoTexture;

    const sf::Texture & levelSelectInactiveTexture;

    const sf::Texture & levelSelectActiveTexture;

    const sf::Texture & levelSelectLockedTexture;
    ...
    const std::unordered_map<std::string, sf::IntRect> & textureRectangles;
